...
static cl::opt<bool> DisableFallbackVectorization(
    "polly-disable-fallback-vectorization",
    cl::desc("Disable the loop vectorizer on original fallback loops after "
             "Polly code generation"),
...
static bool generateCode(Scop &S, IslAstInfo &AI, LoopInfo &LI,
  ...
  // The code below annotates the "llvm.loop.vectorize.enable" to false for the
  // code flow taken when RTCs fail. Because we don't want the Loop Vectorizer
  // to come in later and vectorize the original fall back loop when Polly is
  // enabled. Keep it optional so benchmarking and source-level fusion
  // experiments can distinguish Polly codegen costs from fallback metadata
  // poisoning after later cleanup passes fold the generated path.
  if (DisableFallbackVectorization) {
