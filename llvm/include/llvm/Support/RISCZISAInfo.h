...
namespace llvm {
void risczExtensionsHelp(StringMap<StringRef> DescMap);

class RISCZISAInfo {
public:
  RISCZISAInfo(const RISCZISAInfo &) = delete;
  RISCZISAInfo &operator=(const RISCZISAInfo &) = delete;

  /// Represents the major and version number components of a RISC-Z extension.
  ...
  RISCZISAInfo(unsigned XLen, OrderedExtensionMap &Exts)
  ...
  static llvm::Expected<std::unique_ptr<RISCZISAInfo>>
  ...
  static llvm::Expected<std::unique_ptr<RISCZISAInfo>>
  postProcessAndChecking(std::unique_ptr<RISCZISAInfo> &&ISAInfo);
  static std::string getTargetFeatureForExtension(StringRef Ext);

private:
  RISCZISAInfo(unsigned XLen)
